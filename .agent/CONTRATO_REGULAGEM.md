...
- Etapas da regulagem (nova e salva): `ExpansionTile` no `ProgressiveCard` — círculo 1 / cadeado **e** chevron (sem ✓ verde nas etapas 1–4: `showCompletedMarker: false`). Etapa bloqueada começa recolhida; ao desbloquear, abre sozinha.
...
- Medições: `_PontasLista` (`pontas_table.dart`), lista plana sem chevron — uma linha por ponta; toque no cabeçalho seleciona a ponta ativa (também em `readonly`, só navegação). Cabeçalho com nº, L/min, % e `StatusBadge`; `null` = “Sem medição”, `0` medido é `0.000 L/min` (não some). Corpo da ponta ativa: campo medido (desabilitado em `readonly`) e ideal. Card “Medições das Pontas” fechado lista só pontas já medidas. Ao trocar de ponta, auto-grava (sem fechar a tela) se a etapa 1 estiver completa e não for `readonly`.
