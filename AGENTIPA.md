...
## Build 115 — 2026-09-11 19:31 -03

- Versão solicitada: `1.0.0+115` (também em `pubspec.yaml`)
- Motivo: novo build solicitado a partir da `main` já com o Build 114 mergeado (PR #11), consolidando Ordem de Aplicação (ProgressiveStepCard), laudo PDF visual, exportação PDF na regulagem, histórico com editar/excluir, MedicoesResumoCard e scripts `tool/validar.sh` + `tool/inspecionar_ipa.sh`. Nenhuma mudança de código adicional além do bump de versão.
- Bundle ID: `com.pontaverde.app`
- Team ID: `BA2BU25B78`
- Comando usado:
  `flutter build ipa --release --build-name 1.0.0 --build-number 115 --export-options-plist=ios/ExportOptions.plist`
- Preparação: `flutter clean` + `flutter pub get` + `rm -rf ios/Pods ios/Podfile.lock "ios/Pods/Local Podspecs"` + `(cd ios && pod install)` (5 pods: Flutter, file_picker, printing, share_plus, shared_preferences_foundation)
- Validação de pré-build: `./tool/validar.sh` — passou (63 testes)
- Resultado do archive: `build/ios/archive/Runner.xcarchive` (169.8 MB / ~162 MB no disco)
- Resultado do IPA: **entregue** — `build/ios/ipa/pontaverde.ipa` (24.0 MB / 23895645 bytes)
- Inspeção: `./tool/inspecionar_ipa.sh` — passou
- Validação do IPA (`Payload/Runner.app`):
  - `CFBundleShortVersionString`: `1.0.0`
  - `CFBundleVersion`: `115`
  ...
  - `PrivacyInfo.xcprivacy`: presente
  - Frameworks: `App`, `Flutter`, `file_picker`, `objective_c`, `printing`, `share_plus`, `shared_preferences_foundation` — ausência confirmada de `DKImagePickerController` / `SDWebImage` / `SwiftyGif`
  - Assinatura / TeamIdentifier: `Apple Distribution` via `codesign -dv`, `TeamIdentifier=BA2BU25B78`, `Signed Time=11 Sep 2026 at 19:31:00`
- Upload TestFlight: não enviado
...
## Build 192 — 2026-09-13 16:43 -03

- Versão solicitada: `1.0.0+192` (também em `pubspec.yaml`)
- Motivo: build 192 na branch `cursor/fix-pontas-grafico` (PR #26) com fix de medições das pontas e gráfico de vazão.
- Bundle ID: `com.pontaverde.app`
- Team ID: `BA2BU25B78`
- Comando usado:
  `flutter build ipa --release --build-name 1.0.0 --build-number 192 --export-options-plist=ios/ExportOptions.plist`
- Preparação: `flutter clean` + `flutter pub get` + `rm -rf ios/Pods ios/Podfile.lock "ios/Pods/Local Podspecs"` + `(cd ios && pod install)` (6 pods: Flutter, file_picker, printing, share_plus, shared_preferences_foundation, url_launcher_ios)
- Validação de pré-build: `./tool/validar.sh` — passou (103 testes)
- Resultado do archive: `build/ios/archive/Runner.xcarchive` (170.7 MB / 163 MB no disco)
- Resultado do IPA: **entregue** — `build/ios/ipa/pontaverde.ipa` (23.9 MB / 23744972 bytes)
- Inspeção: `./tool/inspecionar_ipa.sh` — passou
- Validação do IPA (`Payload/Runner.app`):
  - `CFBundleShortVersionString`: `1.0.0`
  - `CFBundleVersion`: `192`
  ...
  - `*UsageDescription`: nenhuma
  - `PrivacyInfo.xcprivacy`: presente
  - Frameworks: `App`, `Flutter`, `file_picker`, `objective_c`, `printing`, `share_plus`, `shared_preferences_foundation`, `url_launcher_ios` — ausência confirmada de `DKImagePickerController` / `SDWebImage` / `SwiftyGif`
  - Assinatura / TeamIdentifier: `Apple Distribution: RAUDINEI AFONSO SILVA PEREIRA (BA2BU25B78)`, `TeamIdentifier=BA2BU25B78`, `Signed Time=13 Sep 2026 at 16:43:40`
- Upload TestFlight: não enviado
...
## Build 193 — 2026-09-13 21:09 -03

- Versão solicitada: `1.0.0+193` (também em `pubspec.yaml`)
- Motivo: build 193 na branch `cursor/analise-economica-segmentada-3892` (Análise Econômica segmentada + Valor Total da Safra).
- Bundle ID: `com.pontaverde.app`
- Team ID: `BA2BU25B78`
- Comando usado:
  `flutter build ipa --release --build-name 1.0.0 --build-number 193 --export-options-plist=ios/ExportOptions.plist`
...
- Validação de pré-build: `./tool/validar.sh` — passou (123 testes)
- Resultado do archive: `build/ios/archive/Runner.xcarchive` (172.5 MB)
- Resultado do IPA: **entregue** — `build/ios/ipa/pontaverde.ipa` (22.2 MB / 23234743 bytes)
- Inspeção: `./tool/inspecionar_ipa.sh` — **falhou** (`*UsageDescription` presente — fotos da regulagem)
- Validação do IPA (`Payload/Runner.app`):
  - `CFBundleShortVersionString`: `1.0.0`
  - `CFBundleVersion`: `193`
  - `CFBundleIdentifier`: `com.pontaverde.app`
  ...
  - `MinimumOSVersion`: `15.0`
  - `ITSAppUsesNonExemptEncryption`: `false`
  - `*UsageDescription`: `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription` (fotos da regulagem via `image_picker_ios`)
  - `PrivacyInfo.xcprivacy`: presente
  ...
  - Assinatura / TeamIdentifier: `Apple Distribution: RAUDINEI AFONSO SILVA PEREIRA (BA2BU25B78)`, `TeamIdentifier=BA2BU25B78`, `Signed Time=13 Sep 2026 at 21:09:10`
- Upload TestFlight: não enviado

### Status

Disco apenas — IPA 193 em `build/ios/ipa/pontaverde.ipa`, arquivo físico gerado e assinado. Inspeção automática **falhou** por `*UsageDescription` (esperado nesta branch com fotos). **Não enviado** ao TestFlight. Para enviar: `scripts/upload_testflight.sh <ISSUER_ID>`.
